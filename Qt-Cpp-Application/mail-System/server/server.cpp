    ...
    ui(new Ui::server)
{
    ...
    this->server1 = new QTcpServer(this);
    //开启监听
    this->server1->listen(QHostAddress::Any,6666);
    connect(this->server1,SIGNAL(newConnection()),this,SLOT(acceptConnection()));
...
{
    //返回一个socket连接
    this->socket = this->server1->nextPendingConnection();
